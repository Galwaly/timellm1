...
import matplotlib.pyplot as plt  # ✅ 新增：绘图模块
...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        val_loss_curve.append(vali_loss)

        accelerator.print(f"Epoch {epoch+1}: Train={train_loss:.4f} | Val={vali_loss:.4f} | Test={test_loss:.4f}")
    ...
    if accelerator.is_local_main_process:
        ...
        np.save(os.path.join(path, 'real_prediction.npy'), trues)
        ...
        plt.plot(preds[idx].flatten(), label='Prediction')
        plt.legend()
        plt.title(f'Prediction vs True (Sample {idx})')
        plt.savefig(os.path.join(path, f'pred_vs_true_{idx}.png'))
        plt.close()

accelerator.wait_for_everyone()
if accelerator.is_local_main_process:
    # del_files('./checkpoints')
    accelerator.print('✅ Experiment completed. Check results in checkpoint folder.')
